    ...
    createUserForm.addEventListener('submit', function (event) {
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                //This contains what comes back from the DB
                userRaw = JSON.parse(xhr.responseText)

                //This is the same as a login
                //Check if the user exists, if we got an ID back from the DB
                if (userRaw[0].ID) {
                    //Yes, then set this user ID in the local storage
                    window.localStorage.setItem('todoUser',userRaw[0].ID);
                }
